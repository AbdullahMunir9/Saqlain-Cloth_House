...
const ProductsCRUD = () => {
    ...
    const handleAddProduct = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post(`${API_BASE_URL}/products`, { name }, {
                headers: { Authorization: `Bearer ${userInfo.token}` }
            });
            setName('');
            fetchProducts();
    ...
    const handleUpdateProduct = async (id) => {
        ...
        setLoading(true);
        try {
            await axios.put(`${API_BASE_URL}/products/${id}`, { name: editName }, {
                headers: { Authorization: `Bearer ${userInfo.token}` }
            });
            setEditingId(null);
            setEditName('');
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            {/* Add Product Form */}
            <form onSubmit={handleAddProduct} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row gap-4">
                <input
                    type="text"
                    placeholder={t('Enter product name (e.g. Cotton, Washing Wear)')}
                    className="flex-1 p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring-1 focus:ring-primary outline-none text-base"
                    value={name}
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="flex items-center justify-center gap-2 px-6 py-3 bg-primary text-white rounded-lg font-semibold hover:bg-primary-dark transition-colors disabled:opacity-50 shadow-md"
                >
                    <Plus size={20} />
                    {t('Add Item')}
                </button>
